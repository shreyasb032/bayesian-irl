...
def analyze(data: dict):
    ...
    fig, ax = plot(fig, ax, mean.flatten(), std.flatten(), 'tab:blue', plot_label='feedback', title="Trust after each site", x_label='Mission index', y_label='Trust')
    
    trust_estimate = data['trust estimate']
    mean = np.mean(trust_estimate, axis=0)
    std = np.std(trust_estimate, axis=0)

    fig, ax = plot(fig, ax, mean.flatten(), std.flatten(), 'tab:orange', plot_label='estimate', title="Trust after each site", x_label='Mission index', y_label='Trust')
